...
import com.prismworks.prism.domain.post.interfaces.dto.PostDto.ViewPostDto;
...
public interface PostControllerDocs {
	...
	ApiSuccessResponse searchRecruitmentPosts(
		...
		@ParameterObject PostDto.SearchRecruitmentPostsRequest request
	);
	...
	@ApiResponses({
		@ApiResponse(responseCode = "200", description = "팀모집 게시글 상세 조회 성공",
			content = @Content(schema = @Schema(implementation = ViewPostDto.class))),
		@ApiResponse(responseCode = "400", description = "잘못된 요청",
			content = @Content(schema = @Schema(implementation = ApiErrorResponse.class))),
	...
	ApiSuccessResponse createRecruitmentPost(
		@Parameter(hidden = true) UserContext userContext,
		PostDto.CreateRecruitmentPostRequest request
	);
	...
	ApiSuccessResponse getBookmarkedRecruitmentPosts(@Parameter(hidden = true) UserContext userContext,
		@ParameterObject PostDto.SearchBookmarkedRecruitmentPostsRequest request);
